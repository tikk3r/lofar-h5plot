...
class H5PlotGUI(QWidget):
    ...
    def __init__(self, h5file, logging_instance, parent=None):
        ...
        if "pol" in self.soltab.getAxesNames():
            self.polarisations = try_get_axis(self.soltab, "pol")
        else:
        ...
        self.stcache = SoltabCache(
            self.soltab.getValues(),
    ...
    def plot(self, labels=None, mode=None, plot_all=False):
        gw = GraphWindow(
            ...
            frametitle="",
            antindex=self.station_picker.currentRow(),
            refantindex=self.refant_picker.currentIndex(),
            axis=self.axis,
            st=self.soltab,
            times=self.times,
        )
