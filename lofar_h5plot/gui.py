...
class H5PlotGUI(QWidget):
    ...
    def _refant_picker_event(self):
        ...
        """
        self.logger.debug(
            "Reference antenna changed to: " + self.refant_picker.currentText()
        )
    ...
    def _phasewrap_event(self):
        ...
        self.logger.debug(
            "Phase wrapping changed to " + str(self.phasewrap_box.isChecked())
        )
    ...
    def _plot_all_button_event(self):
        ...
        if self.axis == "freq" or self.axis == "time":
            ...
            self.plot_waterfall(
                labels=("time", "freq"), mode=self.plotmode, plot_all=True
            )
    ...
    def plot_waterfall(
        ...
        if (
            ("rotationmeasure" in self.soltab.name)
            or ("RMextract" in self.soltab.name)
            or ("clock" in self.soltab.name)
            or ("faraday" in self.soltab.name)
            or ("tec" in self.soltab.name)
        ):
            self.logger.info(
                "Rotation Measure, clock, faraday or TEC cannot be plotted in 2D!"
            )
            return
        self.logger.info("Plotting " + self.soltab.name + " for " + self.solset.name)
        ...
        try:
            ...
            if hasattr(self, "polarizations"):
                if len(self.polarizations) > 1 and self.check_pdiff.isChecked():
                    ...
                    x, y, z1, zw1, p = load_axes_2d(
                        self.stcache.values,
                        self.stcache.weights,
                        self.soltab,
                        antenna=antenna,
                        refantenna=int(np.argwhere(self.stations == self.refant)),
                        pol=0,
                        direction=self.direction,
                    )
                    x2, y2, z2, zw2, p2 = load_axes_2d(
                        self.stcache.values,
                        self.stcache.weights,
                        self.soltab,
                        antenna=antenna,
        ...
        try:
            plot_window = GraphWindow2D(
                self.stcache.values,
                self.stcache.weights,
                self.stations[antenna],
                antenna,
                int(np.argwhere(self.stations == self.refant)),
                self.axis,
                self.soltab,
                times=self.times,
                ...
                mode=mode,
                do_timediff=self.check_tdiff.isChecked(),
                do_freqdiff=self.check_fdiff.isChecked(),
                do_poldiff=self.check_pdiff.isChecked(),
            )
        except AttributeError:
        ...
        if plot_all:
            ...
            if mode == "values":
                plot_window.plot(
                    ...
                    y,
                    z,
                    ax_labels=("Time [s]", "Freq. [MHz]"),
                    isphase=p,
                    frametitle=self.stations[antenna],
                )
            elif mode == "weights":
                plot_window.plot(
                    x,
    ...
    def plot(
        ...
        if (
            ...
            self.logger.info(
                "Rotation Measure or clock does not support frequency axis! Switch to time instead."
            )
            return
        msg = load_axes(
            self.stcache.values,
        ...
        if self.axis.lower() == "time" and (
            self.frequencies is None or (len(self.frequencies) == 1)
        ):
            plot_window.button_next.setEnabled(False)
